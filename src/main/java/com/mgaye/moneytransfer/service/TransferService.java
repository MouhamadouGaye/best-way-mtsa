    ...
    private void logTransferAuditEvents(User fromUser, User toUser, Transfer transfer, boolean fromCard) {
        try {
            ...
            auditService.logTransactionEvent(
                    fromUser.getId(),
                    "TRANSFER_SENT",
                    String.format("Transfer to %s (%s)", toUser.getName(), toUser.getEmail()),
                    transfer.getId(),
                    transfer.getAmount(),
                    "USD");
            ...
            auditService.logTransactionEvent(
                    toUser.getId(),
                    "TRANSFER_RECEIVED",
                    String.format("Transfer from %s (%s)", fromUser.getName(), fromUser.getEmail()),
                    transfer.getId(),
                    transfer.getAmount(),
                    "USD");
            ...
            if (isLargeTransfer(transfer.getAmount())) {
                auditService.logSecurityEvent(
                        ...
                        String.format("Large transfer of %s USD to %s",
                                transfer.getAmount(), toUser.getName()));
            }
